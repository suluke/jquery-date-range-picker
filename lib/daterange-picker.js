...
class DaterangePicker {
	...
	initDatepicker() {
		...
		this.registerOpenCloseListeners(opt.openListeners, opt.closeListeners);
	...
	registerOpenCloseListeners(openListeners, closeListeners) {
		const box = this.box;
		const opt = this.opt;
		...
		const isOpenStateChangedBy = function(evt) {
			...
			return evt[eventChangesOpenStateTag] && evt[eventChangesOpenStateTag].indexOf(box.get(0)) >= 0;
		};
	...
	invalidate() {
		...
		this.showSelectedDays(this.opt);
	}
	...
	createApi() {
		...
		this.api = {
			setDateRange: (d1, d2, silent) => {
				...
				this.setDateRange(d1, d2, silent);
			},
			clear: () => this.clearSelection(),
			clearEnd: () => this.clearEnd(),
			close: () => this.closeDatePicker(),
			open: () => this.openDatePicker(),
			getDatePicker: () => this.box,
			destroy: () => {
				this.unregisterListeners(opt.openListeners);
				this.unregisterListeners(opt.closeListeners);
				this.box.remove();
