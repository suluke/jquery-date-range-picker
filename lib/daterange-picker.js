...
class DaterangePicker {
	...
	triggerHasChanged(forceValid) {
		...
		event.startText = event.date1 ? calendar.getDateString(event.date1, opt.format) : '...';
		event.value = event.startText;
		event.endText = event.date2 ? calendar.getDateString(event.date2, opt.format) : '...';
		event.value += (event.date2 ? opt.separator + event.endText : '');
		...
		if ((range.start && opt.singleDate) || (range.start && range.end)) {
			...
			event.valid = true;
			if (range.end) {
				event.selectedText = '' + calendar.countDays(range.end, range.start);
			}
