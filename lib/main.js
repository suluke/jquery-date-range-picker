...
const DaytimeSelect = require('./daytime-range');
...
$.fn.dateRangePicker = function(opt) {
	...
	$(this).data('dateRangePicker', {
		...
		getDatePicker: () => box,
		destroy: function() {
			func.unregisterOpenListeners(self);
			func.unregisterCloseListeners(self);
			$(self).off('.datepicker');
	...
	function initDatepicker(element) {
		...
		if (opt.showTopbar) {
			topBar = new TopBar(box, opt.language, opt.autoClose, opt.singleDate, opt.customTopBar, opt.separator, opt.applyBtnClass, () => {
				closeDatePicker();
				var dateRange = (
		...
		if (DaytimeSelect.addTimeRangeSelectToPicker) {
			const daytimes = DaytimeSelect.addTimeRangeSelectToPicker(opt, box, invalidate);
			daytime1 = daytimes[0];
			daytime2 = daytimes[1];
		}
				...
				if (opt.start && !opt.end) {
					...
					if (func.hoveringTooltip) {
						if (typeof func.hoveringTooltip === 'function') {
							tooltip = func.hoveringTooltip(days, opt.start, hoverTime, opt);
	...
	function setDateRange(date1, date2, silent) {
		...
		if (opt.time.enabled) {
			...
			daytime2.renderTime('time2', date2);
		}
