...
$.fn.dateRangePicker = function(opt) {
	...
	function initDatepicker($element) {
		...
		if (opt.showTopbar) {
			topBar = TopBar.addToPicker(box, opt, () => {
				...
				$element.trigger('datepicker-apply', {
					'value': dateRange,
		...
		if (opt.draggableRangeBounds) {
			DraggableRangeBounds.addToPicker(box, (start, end) => {
				setDateRange(start, end, false, true);
			});
		}
		if (opt.scrollThroughMonths) {
		...
		if (opt.time.enabled) {
			const daytimes = DaytimeSelect.addToPicker(opt, opt.time, opt.language, opt.singleDate, opt.defaultTime, box, invalidate);
			daytime1 = daytimes[0];
			daytime2 = daytimes[1];
		}
	 ...
	 */
	function setDateRange(date1, date2, silent, keepMonthPositions) {
