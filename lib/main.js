...
const DaytimeRange = require('./daytime-range');
...
$.fn.dateRangePicker = function(opt) {
	...
	function showSelectedInfo(forceValid, silent) {
		...
		if ((opt.start && opt.singleDate) || (opt.start && opt.end)) {
			...
			// Trigger events
			let event = {
				'value': calendar.getDateString(new Date(opt.start), opt.format),
				'date1': new Date(opt.start)
			};
			if (opt.start && opt.end) {
				box.find('.selected-days').show().find('.selected-days-num').html(calendar.countDays(opt.end, opt.start));
				event.value += opt.separator + calendar.getDateString(new Date(opt.end), opt.format);
