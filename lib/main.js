...
$.fn.dateRangePicker = function(opt) {
	...
	var func = {
		...
		calcPosition: function(self, opt, evt) {
		...
		registerOpenListeners: function(self, callback) {
			$(self).bind('click.datepicker', function(evt) {
				callback();
			});
		}
	};
	...
	func.registerOpenListeners(self, function() {
	...
	{
		if (!opt.inline) {
			var positionCss = func.calcPosition(self, opt, evt);
			box.css(positionCss);
		}
	}
	...
	{
		var __default_string = func.getValue.call(selfDom);
	...
	function showSelectedInfo(forceValid,silent)
	{
		...
		if (opt.start && opt.singleDate)
		{
			...
			func.setValue.call(selfDom, dateRange, getDateString(new Date(opt.start)), getDateString(new Date(opt.end)));
	...
	function createMonthHTML(d)
	{
		...
		for(var week=0; week<6; week++)
		{
			...
			for(var day = 0; day<7; day++)
			{
				...
				{
					html.push('<td><div class="week-number" data-start-time="'+today.time+'">'+func.getWeekNumber(today.date)+'</div></td>');
				}
