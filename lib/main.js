...
$.fn.dateRangePicker = function(opt) {
	...
	func.registerOpenListeners(self, function(evt) {
		...
		if (!isOpen && !evt.originalEvent[eventChangesOpenStateTag]) {
			evt.originalEvent[eventChangesOpenStateTag] = true;
	...
	function initDatepicker(element) {
		...
		func.registerCloseListeners(self, function(evt) {
			if (box.is(':visible') && !evt.originalEvent[eventChangesOpenStateTag]) {
				evt.originalEvent[eventChangesOpenStateTag] = true;
				closeDatePicker(evt);
			}
		});
		...
		function gotoPrevMonth(self) {
			...
			if (isMonth2 && calendar.compareMonth(month, opt.month1) <= 0 || isMonthOutOfBounds(month)) {
				return;
			}
			showMonth(month, isMonth2 ? 'month2' : 'month1');
			showGap(opt);
		...
		box.find('.time1 input[type=range]').on('change mousemove', function(e) {
			var target = e.target,
		...
		box.find('.time2 input[type=range]').on('change mousemove', function(e) {
			var target = e.target,
				hour = target.name === 'hour' ? $(target).val().replace(/^(\d{1})$/, '0$1') : undefined,
				min = target.name === 'minute' ? $(target).val().replace(/^(\d{1})$/, '0$1') : undefined;
	...
	function setTime (name, hour, minute) {
		if (hour) {
			box.find('.' + name + ' .hour-val').text(hour);
		}
		if (minute) {
			box.find('.' + name + ' .minute-val').text(minute);
		}
		switch (name) {
			case 'time1':
				if (opt.start) {
