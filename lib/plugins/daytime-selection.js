...
class DaytimeSelect {
	...
	setTimeDisplay(h, m) {
		// Add this to `utils` module if needed somewhere else
		const pad = (str, length, padChar = '0') => {
			return Array(Math.max(length - String(str).length + 1, 0)).join(padChar) + str;
		};
		this.$hourVal.text(pad(h, 2));
		this.$minuteVal.text(pad(m, 2));
	}
	...
	static addToPicker(picker) {
		...
		const dateRange = picker._getWritableRange();
		let daytime1 = null;
		let daytime2 = null;
