...
class JqueryDaterangePicker {
	...
	registerListeners() {
		if (this.options.setupOpenCloseListeners && !this.options.openListeners && !this.options.closeListeners) {
	...
	loadDatesExternally() {
		...
		let fmt = opt.format;
		const loc = opt.language;
		const datesString = opt.getValue();
		const dates = datesString ? datesString.split(opt.separator) : [];

		if (dates.length >= 1) {
			if (fmt.match(/Do/)) {
				// normalize 1st, 2nd... to improve parseability
				fmt = fmt.replace(/Do/, 'D');
				for (let i = 0; i < dates.length; i++) {
					dates[i] = dates[i].replace(/(\d+)(th|nd|st)/, '$1');
				}
			}
			const m1 = moment(dates[0], fmt, loc);
			if (dates.length >= 2) {
	...
	createApi() {
		...
		const singleDate = picker.getOptions().singleDate;
		const $this = this.$elm;
		// Expose some api
		const api = {
			setDateRange: (d1, d2, silent) => {
				...
				if (singleDate) {
					picker.setSingleDate(d1, silent);
				} else {
		...
		$.fn.dateRangePicker = function(options) {
			...
			return this.each((idx, elm) => {
				...
				if ($this.data('dateRangePicker')) {
					console.warn('Prevented creating daterange picker twice on same element');
					return;
				}
