...
class TopBar {
	...
	static addToPicker(picker) {
		...
		const topbar = new TopBar(
			...
			() => {
				...
				let dateRange = '';
				dateRange += opt.start ? moment(opt.start).format(opt.format) : '...';
				dateRange += opt.separator;
				dateRange += opt.end ? moment(opt.end).format(opt.format) : '...';
				picker.getEventEmitter().trigger('datepicker-apply', {
					'value': dateRange,
					'date1': opt.start ? new Date(opt.start) : null,
					'date2': opt.end  ? new Date(opt.end) : null
				});
			}
		);
		if (opt.alwaysOpen) {
	...
	static addOptionDefaults(opt) {
		opt.topBar = {
			enabled: true,
			customText: false, // can be string or function
			applyBtnClass: ''
		};
	}
}
