...
class TopBar {
	...
	static addToPicker(picker) {
		...
		var topbar = new TopBar(picker.getDom(), opt.language, opt.autoclose, opt.singleDate, opt, opt.customTopBar, opt.separator, opt.applyBtnClass,
			() => {
		...
		picker.getEventEmitter().on('datepicker-show-info', data => {
			topbar.setState(data.startText, data.endText, data.selectedText, data.valid);
		});
