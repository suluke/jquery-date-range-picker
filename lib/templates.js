...
const getWeekHead = (startOfWeek, showWeekNumbers, lang) => {
	let html = [];
	let day = moment().locale(lang).day(0); // set `day` to any sunday
	for (let i = 0; i < 7; i++) {
		// TODO the 'substring' is just a small hack to enforce previous behavior
		html.push(`<th>${day.format('ddd').substring(0,2)}</th>`);
		day.add(1, 'd');
	}
...
module.exports = {
	createDom: function(opt) {
		...
		html += '</div>'; // Month-wrapper ends here
