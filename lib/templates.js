...
module.exports = {
		...
		if (opt.showTopbar)
		{
			...
			else
			{
				html += '<div class="normal-top">\
						<span style="color:#333">'+i18n.lang('selected', opt.language)+' </span> <b class="start-day">...</b>';
				if ( ! opt.singleDate ) {
					html += ' <span class="separator-day">'+opt.separator+'</span> <b class="end-day">...</b> <i class="selected-days">(<span class="selected-days-num">3</span> '+i18n.lang('days', opt.language)+')</i>'
				}
				html += '</div>';
			...
			html += '<input type="button" class="apply-btn disabled'+ module.exports.getApplyBtnClass(opt.autoClose, opt.applyBtnClass) +'" value="'+i18n.lang('apply', opt.language)+'" />';
			html += '</div>'
		}

		var _colspan = opt.showWeekNumbers ? 6 : 5;
		html += '<div class="month-wrapper">'
			+'<table class="month1" cellspacing="0" border="0" cellpadding="0"><thead><tr class="caption"><th style="width:27px;"><span class="prev">&lt;</span></th><th colspan="'+_colspan+'" class="month-name"></th><th style="width:27px;">' + (opt.singleDate || !opt.stickyMonths ? '<span class="next">&gt;</span>': '') + '</th></tr><tr class="week-name">'+module.exports.getWeekHead(opt.startOfWeek, opt.showWeekNumbers, opt.language)+'</thead><tbody></tbody></table>';

		if ( showSecondMonth(opt) )
		{
			html += '<div class="gap">' + module.exports.getGapHTML() + '</div>'
				+'<table class="month2" cellspacing="0" border="0" cellpadding="0"><thead><tr class="caption"><th style="width:27px;">' + (!opt.stickyMonths ? '<span class="prev">&lt;</span>': '') + '</th><th colspan="'+_colspan+'" class="month-name"></th><th style="width:27px;"><span class="next">&gt;</span></th></tr><tr class="week-name">'+module.exports.getWeekHead(opt.startOfWeek, opt.showWeekNumbers, opt.language)+'</thead><tbody></tbody></table>'
		}
			//+'</div>'
	...
	getWeekHead: function(startOfWeek, showWeekNumbers, lang) {
		...
		if (startOfWeek == 'monday') {
			return prepend+'<th>'+i18n.lang('week-1', lang)+'</th>\
			...
			return prepend+'<th>'+i18n.lang('week-7', lang)+'</th>\
	...
	createMonthHTML: function(d, opt) {
		...
		if (dayOfWeek > 0)
		{
			for (var i = dayOfWeek; i > 0; i--)
			{
				...
				if (opt.startDate && calendar.compare_day(day,opt.startDate) < 0) valid = false;
				if (opt.endDate && calendar.compare_day(day,opt.endDate) > 0) valid = false;
				days.push(
				{
					date: day,
					type:'lastMonth',
		...
		for(var i=0; i<40; i++)
		{
			...
			if (opt.startDate && calendar.compare_day(today,opt.startDate) < 0) valid = false;
			if (opt.endDate && calendar.compare_day(today,opt.endDate) > 0) valid = false;
			days.push({
				date: today,
		...
		for(var week=0; week<6; week++) {
			...
			for(var day = 0; day<7; day++)
			{
				...
				{
					html.push('<td><div class="week-number" data-start-time="'+today.time+'">'+calendar.getWeekNumber(today.date)+'</div></td>');
				}

				html.push('<td ' + attributesCallbacks({}, opt.dayTdAttrs, today) + '><div ' + attributesCallbacks(todayDivAttr, opt.dayDivAttrs, today) + '>'+module.exports.renderDayHTML(today.time, today.day, opt)+'</div></td>');
			}
			html.push('</tr>');
		}
		return html.join('');
