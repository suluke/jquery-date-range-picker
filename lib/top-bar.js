...
class TopBar {
	constructor($container, lang, autoClose, singleDate, intervalLimits, customMarkup, dateSeparator, customCloseBtnClass, onClose, onPrev, onNext) {
	...
	registerListeners() {
		...
		this.$container.find('.next').click(this.onNext);
		this.$container.find('.prev').click(this.onPrev);
	}
	...
	static addToPicker(box, opt, onClose, onPrev, onNext) {
		var topbar = new TopBar(box, opt.language, opt.autoclose, opt.singleDate, opt, opt.customTopBar, opt.separator, opt.applyBtnClass, onClose, onPrev, onNext);
		if (opt.alwaysOpen) {
			topbar.hideCloseBtn();
		}
		return topbar;
